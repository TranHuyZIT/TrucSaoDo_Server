...
const ThongKeController = {
  ...
  getDiemTruTheoSo: async (req, res) => {
    try {
      const MA_SO = req.query.MA_SO;
      const sql = `SELECT scd.MA_SO, L_TEN, sum(vp.DIEM_TRU) as DIEM_TRU  FROM so_co_do as scd 
      join chi_tiet_so_co_do as ctscd on scd.MA_SO
      join chi_tiet_vi_pham as ctvp on ctvp.NGAY = ctscd.ngay and scd.MA_SO = ctvp.MA_SO
      join vi_pham as vp on ctvp.VP_MA = vp.VP_MA
      where scd.MA_SO = ${MA_SO}
      group by scd.L_TEN, scd.MA_SO
      order by DIEM_TRU;`;
